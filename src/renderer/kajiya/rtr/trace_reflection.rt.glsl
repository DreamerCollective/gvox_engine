...
DAXA_DECL_PUSH_CONSTANT(RtrTraceRtPush, push)
...
#define HALFRES_SUBSAMPLE_INDEX (deref(push.uses.gpu_input).frame_index & 3)
    ...
    if (0.0 == depth) {
        safeImageStore(push.uses.out0_tex, ivec2(px), vec4(0.0.xxx, -SKY_DIST));
        return;
    }
    ...
    vec2 urand = vec2(
        blue_noise_sampler(int(px.x), int(px.y), int(noise_offset), 0,
                           push.uses.ranking_tile_buf,
                           push.uses.scambling_tile_buf,
                           push.uses.sobol_buf),
...
// TODO: move this to a separate sample preparation compute shader
    ...
    if (is_valid(brdf_sample)) {
        ...
        // uint rng = hash2(px);
        safeImageStore(push.uses.rng_out_tex, ivec2(px), uvec4(rng));
        RtrTraceResult result = do_the_thing(px, gbuffer.normal, gbuffer.roughness, rng, outgoing_ray);

        const vec3 direction_vs = direction_world_to_view(push.uses.gpu_input, outgoing_ray.Direction);
        ...
        safeImageStore(push.uses.out0_tex, ivec2(px), vec4(result.total_radiance, rtr_encode_cos_theta_for_fp16(cos_theta)));
        safeImageStore(push.uses.out1_tex, ivec2(px), vec4(hit_offset_ws, pdf));
        safeImageStore(push.uses.out2_tex, ivec2(px), vec4(result.hit_normal_vs, 0));
    } else {
        safeImageStore(push.uses.out0_tex, ivec2(px), vec4(vec3(1, 0, 1), 0));
        safeImageStore(push.uses.out1_tex, ivec2(px), 0.0.xxxx);
    }
}
#endif
